...
class Root{
    ...
    * @return
    */
   public Node addNode(float x, float y, float z, boolean addToBase) {
      if (!addToBase) {
         lastNode = new Node(x, y, z, lastNode, true);
         if (nNodes == 0) firstNode = lastNode;
         nNodes++;
         return lastNode;
         }
      else {
         firstNode = new Node(x, y, z, firstNode, false);
   ...
   public void readRSML(org.w3c.dom.Node parentDOM, RootModel rm, Root parentRoot, String origin) {
				   ...
				   if (geomName.equals("polyline")) {
					   ...
					   while (nodePoint != null) {
						   ...
						   if (pointName.equals("point")) {
							   	if(counter == clock){
							   		Node no = addNode(0.0f, 0.0f, 0.0f, false);
      ...
      while(n != null){
    	  if(n.diameter == 0){
    		  //System.out.println("Recenter node");

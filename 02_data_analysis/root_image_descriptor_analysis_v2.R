...
# CREATE PAIRWISE PLOTS BASED ON THE HEAT MAP
pair <-c("tot_root_length", "tip_count", "monocot")
temp <- data.frame(x=data[[pair[1]]], y=descr[[pair[2]]], group=data$specie)
ggplot(temp[temp$group==pair[3],], aes(x, y)) + 
  geom_point(colour="grey50") + 
  stat_smooth(method="lm", se=FALSE, colour="black") + 
  geom_abline(intercept = 0, slope = 1, lty=2) + 
  xlab(paste0("\n measured ", pair[1], " [cm]\n")) + 
  ylab(paste0("estimated ",pair[2], " [-]\n")) + 
  theme_classic() + 
  theme(legend.position = "none", text=element_text(size=20), 
        axis.line.x = element_line(colour = 'black', size=0.5, linetype='solid'),
        axis.line.y = element_line(colour = 'black', size=0.5, linetype='solid')) +
  ggsave(paste0(base.dir,"/00_data/figures/plot-",pair[1],"-",pair[2],"-",pair[3],".pdf"), width = 5, height=5)


pair <-c("mean_prim_length", "depth", "dicot")
temp <- data.frame(x=data[[pair[1]]], y=descr[[pair[2]]], group=data$specie)
ggplot(temp[temp$group==pair[3],], aes(x, y)) + 
  geom_point(colour="grey50") + 
  stat_smooth(method="lm", se=FALSE, colour="black") + 
  geom_abline(intercept = 0, slope = 1, lty=2) + 
  xlab(paste0("\n measured ", pair[1], " [cm]\n")) + 
  ylab(paste0("estimated ",pair[2], " [cm]\n")) + 
  theme_classic() + 
  theme(legend.position = "none", text=element_text(size=20), 
        axis.line.x = element_line(colour = 'black', size=0.5, linetype='solid'),
        axis.line.y = element_line(colour = 'black', size=0.5, linetype='solid')) +
...
pair <-c("n_laterals", "length", "monocot")
temp <- data.frame(x=data[[pair[1]]], y=descr[[pair[2]]], group=data$specie)
ggplot(temp[temp$group==pair[3],], aes(x, y)) + 
  geom_point(colour="grey50") + 
  stat_smooth(method="lm", se=FALSE, colour="black") + 
...
pair <-c("tot_root_length", "area", "dicot")
temp <- data.frame(x=data[[pair[1]]], y=descr[[pair[2]]], group=data$specie)
ggplot(temp[temp$group==pair[3],], aes(x, y)) + 
  geom_point(colour="grey50") + 
  stat_smooth(method="lm", se=FALSE, colour="black") + 
  geom_abline(intercept = 0, slope = 1, lty=2) + 
  xlab(paste0("\n measured ", pair[1], " [cm]\n")) + 
  ylab(paste0("estimated ",pair[2], " [cm2]\n")) + 
  theme_classic() + 
  theme(legend.position = "none", text=element_text(size=20), 
        axis.line.x = element_line(colour = 'black', size=0.5, linetype='solid'),
        axis.line.y = element_line(colour = 'black', size=0.5, linetype='solid')) +
